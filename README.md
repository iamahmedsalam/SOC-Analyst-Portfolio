...
### ☁️ [Azure Sentinel SOC Lab](https://github.com/iamahmedsalam/azure-sentinel-soc-lab)

A hybrid SIEM evaluation — the same multi-VM home SOC lab, instrumented end-to-end in Microsoft Sentinel, with live side-by-side comparison drills against the existing Wazuh deployment. Not a tool shown in isolation: same attacks, same hosts, real timestamps, real bugs found and fixed on both platforms.

**What I built:**
- 7 custom KQL Analytics Rules engineered from scratch, 5 deployed live + 2 with mechanics validated pending sufficient historical data
- 4 live comparison drills — real attacks re-run against both SIEMs simultaneously, not simulated or compared after the fact
- 2 Sentinel workbooks, including a parameter-driven Entity Investigation tool with genuine cross-platform (Windows + Linux) process correlation
- A complete Linux telemetry pipeline (auditd + audisp-syslog) built from scratch mid-project to close a real visibility gap I identified myself

**Key findings:**
- Found and fixed 3 real detection bugs live: a Sentinel regex that missed an abbreviated PowerShell flag, a missing LOLBIN in a correlation rule (caught before running the drill), and a KQL `has`-vs-`contains` bug that silently failed to match hex-encoded data
- Diagnosed a genuine ~50-minute Azure Monitor Agent authentication outage with primary-source log evidence, rather than assuming "cloud forwarding is unreliable"
- Discovered a real, undocumented architecture distinction: Azure's legacy `SecurityEvent` table is never populated by the modern Azure Monitor Agent
- Honest, two-directional comparison: Wazuh detects in under a second; Sentinel's KQL expresses cross-event correlations Wazuh's rule engine structurally can't

| | |
|---|---|
| **Repo** | [github.com/iamahmedsalam/azure-sentinel-soc-lab](https://github.com/iamahmedsalam/azure-sentinel-soc-lab) |
| **KQL Rules Reference** | [kql-queries-reference.md](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/detection-rules/kql-queries-reference.md) |
| **Live Drills** | [Drill 1](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/drills/drill-01-ssh-bruteforce.md) · [Drill 2](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/drills/drill-02-encoded-powershell.md) · [Drill 3](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/drills/drill-03-lolbin-execution.md) · [Drill 4](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/drills/drill-04-linux-lolbin.md) |
| **Workbooks** | [Detections Overview](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/workbooks/workbook-reference.md) · [Entity Investigation](https://github.com/iamahmedsalam/azure-sentinel-soc-lab/blob/main/workbooks/entity-investigation-reference.md) |
...
**Detection Engineering:** Custom Wazuh rule authoring (25 rules), PCRE2 regex, MITRE ATT&CK mapping, Sysmon event ID analysis, `if_sid` vs `if_group` chaining, XML validation workflow

**Cloud SIEM & KQL:** Microsoft Sentinel Analytics Rules and Workbooks, KQL query engineering (joins, `make-series`, statistical anomaly detection), Azure Monitor Agent log forwarding and Data Collection Rules, infrastructure-as-code rule deployment via Azure CLI, Linux audit instrumentation (auditd, audisp-syslog)
